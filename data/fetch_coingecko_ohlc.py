...
def fetch_coingecko_ohlc(coin_id, days=1):
    ...
    try:
        ...
        # CoinGecko returns [ts, o, h, l, c] (5 fields) — normalize to 6 (volume=None)
        normalized = []
        for row in data:
            if len(row) == 5:
                ts, o, h, l, c = row
...
def normalize_gecko(candles):
    ...
    for ts in sorted(grouped.keys()):
        ...
        clean.append([
            ts * 1000,  # convert seconds -> ms to match CoinGecko
            group[0][0],
...
def save_ohlc(pair_id, rows):
    ...
    for row in rows:
        ...
        # handle both 5-field (CoinGecko raw) and 6-field (normalized)
        if len(row) == 5:
            ts, o, h, l, c = row
            v = None
        elif len(row) == 6:
...
def main():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT contract, pair_id FROM supported_tokens WHERE contract IS NOT NULL AND pair_id IS NOT NULL"
    )
    tokens = cur.fetchall()
    conn.close()
    ...
    for contract, pair_id in tokens:
        process_token(contract, pair_id)
        time.sleep(2)
