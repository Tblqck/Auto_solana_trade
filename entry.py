...
from LossGuard import run_loss_guard
...
def run_lossguard_cycle():
    print("⏱️ Running LossGuard...\n")

    conn = get_db_connection2()
    now = pd.Timestamp.utcnow()

    # ============================
    # RESULT BUCKETS (IMPORTANT)
    # ============================
    results = {
        "safe": [],
        "safe_hold": [],
    ...
    try:
        ...
        for pair_id in candidates:
            ...
            if status in ("SAFE", "BLOCKED") and seconds_passed < INTERVAL:
                print(f"⏭️ HOLD ({status}, cooldown): {pair_id}")
        ...
        print(f"\n🟢 SAFE PAIRS COUNT: {len(safe_pairs)}")
